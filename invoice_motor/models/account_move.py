            ...
            for line_sale in line.sale_line_ids:
                for line_move in line_sale.move_ids:
                    ...
                    for linea in stock_move_line:
                        ...
                        pedimento = self.env['stock.landed.cost'].search([('picking_ids', 'in', linea.move_id.id)]).l10n_mx_edi_customs_number
                        date_1 = self.env['stock.landed.cost'].search([('picking_ids', 'in', linea.move_id.id)]).date
